...
public string Encrypt(string text = "Two One Nine Two", string key = "Thats my Kung Fu")  
    ...
    private void MixColumn(byte[,] stateArray)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            temp[0, i] = (byte)(M2[stateArray[0, i]] ^ M3[stateArray[1, i]] ^ stateArray[2, i] ^ stateArray[3, i]);
            temp[1, i] = (byte)(stateArray[0, i] ^ M2[stateArray[1, i]] ^ M3[stateArray[2, i]] ^ stateArray[3, i]);
            temp[2, i] = (byte)(stateArray[0, i] ^ stateArray[1, i] ^ M2[stateArray[2, i]] ^ M3[stateArray[3, i]]);
            temp[3, i] = (byte)(M3[stateArray[0, i]] ^ stateArray[1, i] ^ stateArray[2, i] ^ M2[stateArray[3, i]]);
        }
